...
function App() {
  ...
  const [maybeEngineUnavailableMessage, setMaybeEngineUnavailableMessage] = useState<string | null>(null)
  ...
  useEffect(() => {
    const initEngines = async () => {
      ...
      if (defaultEngine) {
        ...
        setMaybeEngineUnavailableMessage(null)
        ...
        const availableExportEngines = availableEngines.filter(
          (e) => e.available && e.supportsExport
        )

        if (availableExportEngines.length === 0) {
          setMaybeEngineUnavailableMessage(
            'On-device audiobook export isn’t supported in this browser/device. For best results, try the latest Chrome or Edge on desktop. Cloud export (works everywhere) is coming soon.'
          )
        } else {
          setMaybeEngineUnavailableMessage(
            'No export-capable voice engine is available right now. Try reloading the page, or use the latest Chrome or Edge on desktop. Cloud export is coming soon.'
          )
        }
      }
  ...
  const canGenerate = canExport && !!text.trim() && !!selectedVoice
  const showEngineUnavailableBanner = !isLoadingEngines && !selectedEngine && !!maybeEngineUnavailableMessage
      ...
      <main className="max-w-6xl mx-auto px-6 py-5">
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            {showEngineUnavailableBanner && (
              <div className="glass-panel p-4 bg-amber-900/20 border border-amber-600/30 rounded-lg animate-fade-in">
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 mt-0.5">
                    <WarningIcon />
                  </div>
                  <div className="flex-1 text-sm">
                    <p className="font-medium text-amber-200 mb-1">
                      TTS export unavailable
                    </p>
                    <p className="text-amber-300/80 leading-relaxed">
                      {maybeEngineUnavailableMessage}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <ControlPanel
              ...
              maybeDisabledReason={
                showEngineUnavailableBanner ? 'On-device audiobook export is unavailable on this browser/device.' : null
              }
              canDownload={!!pcmAudio}
              m4bSupported={m4bSupported}
            />
